...
const translations = {
  ...
  ar: {
    ...
    hero: {
      ...
      title2: "المالي",
      title3: "الموثوق",
      desc: "تقدم مباشر كابيتال القابضة خبرات لا مثيل لها في الخدمات المصرفية الاستثمارية، وإدارة الأصول، وخدمات الوساطة في جميع أنحاء الشرق الأوسط.",
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-10 flex items-center justify-between">
          ...
          <a href="/" className="group flex items-center gap-2.5 no-underline" style={{cursor:"none"}}>
            ...
            <div className="flex flex-col leading-none">
              <span className={`font-heading font-extrabold text-lg tracking-tight text-foreground ${lang === 'ar' ? 'order-1' : ''}`}>
                MUBASHER
              </span>
          ...
          <div className="hidden lg:flex items-center gap-8">
            {links.map(l => (
              <a key={l.id} href={`#${l.id}`} className="nav-link">{l.name}</a>
            ))}
          </div>
          ...
          <div className="flex items-center gap-3">
            <button 
              ...
              className="w-9 h-9 rounded-full border border-border/50 bg-secondary/50 backdrop-blur-md flex items-center justify-center text-muted-foreground hover:text-foreground transition-all duration-300"
              style={{cursor:"none"}}
              data-hover
            >
            ...
            <ThemeToggle />
            <a href="#contact" className="btn-primary hidden md:inline-flex text-[10px] uppercase font-bold tracking-widest h-10 px-6" style={{cursor:"none"}}>
              {t.nav.clientPortal}
            </a>
            <button
    ...
    <section ref={ref} className="relative min-h-[100svh] flex flex-col items-center justify-center text-center px-6 pt-28 overflow-hidden noise">
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <div className="overflow-hidden mb-6">
          <motion.h1
            className={`font-heading ${lang === 'ar' ? 'text-[clamp(2.5rem,8vw,6rem)]' : 'text-[clamp(3.5rem,10vw,8.5rem)]'} font-extrabold leading-[0.95] tracking-tight`}
            initial={{ y: "110%" }}
            animate={{ y: 0 }}
        ...
        >
          <a href="#services" className="btn-primary min-w-[220px]">
            {t.hero.primaryBtn} {lang === 'en' ? <ArrowRight className="w-4 h-4" /> : null}
          </a>
      ...
      >
        <div className="text-center">
          ...
          <div className="font-sans text-[10px] tracking-[0.2em] text-muted-foreground uppercase mt-1">{t.hero.stat1}</div>
        </div>
        <div className="text-center">
          <div className="font-heading text-2xl md:text-4xl font-extrabold">48+</div>
          <div className="font-sans text-[10px] tracking-[0.2em] text-muted-foreground uppercase mt-1">{t.hero.stat2}</div>
        </div>
        <div className="text-center">
          <div className="font-heading text-2xl md:text-4xl font-extrabold">80+</div>
          <div className="font-sans text-[10px] tracking-[0.2em] text-muted-foreground uppercase mt-1">{t.hero.stat3}</div>
        </div>
      </motion.div>
    </section>
  );
...
function AboutSection({ t, lang }: { t: any, lang: string }) {
  return (
    <section id="about" className="py-32 px-6 bg-secondary/30 relative z-10 overflow-hidden">
      <div className={`max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-20 ${lang === 'ar' ? 'lg:flex-row-reverse text-right' : ''}`}>
        <motion.div
          className="lg:w-1/2"
        ...
        >
          ...
          <h2 className="font-heading text-[clamp(2.5rem,5vw,4rem)] font-extrabold tracking-tight mt-4 mb-8 leading-[1.1]">
            {t.about.title}
          </h2>
          ...
          <div className="grid grid-cols-2 gap-8 mb-10">
            <div>
              ...
              <div className="font-sans text-[10px] tracking-wider text-muted-foreground uppercase mt-1">{t.about.stat1}</div>
            </div>
            <div>
              <div className="font-heading text-3xl font-bold text-foreground">500+</div>
              <div className="font-sans text-[10px] tracking-wider text-muted-foreground uppercase mt-1">{t.about.stat2}</div>
            </div>
          </div>
          <a href="#contact" className="btn-primary">{t.about.btn}</a>
        </motion.div>
        ...
        >
          <div className="aspect-[4/5] rounded-[2rem] bg-gradient-to-br from-blue-600/20 to-blue-800/10 border border-border/50 overflow-hidden relative">
            ...
            <div className={`absolute bottom-6 font-heading font-bold text-xl ${lang === 'en' ? 'left-6' : 'right-6'}`}>{lang === 'en' ? 'Cairo' : 'القاهرة'}</div>
          </div>
          <div className="aspect-[4/5] rounded-[2rem] bg-gradient-to-br from-emerald-500/20 to-emerald-700/10 border border-border/50 mt-12 overflow-hidden relative">
            <div className="absolute inset-0 grid-overlay opacity-20" />
            <div className={`absolute bottom-6 font-heading font-bold text-xl ${lang === 'en' ? 'left-6' : 'right-6'}`}>{lang === 'en' ? 'Dubai' : 'دبي'}</div>
          </div>
        </motion.div>
      </div>
    </section>
...
function TrackRecord({ t, lang }: { t: any, lang: string }) {
  ...
  return (
    <section id="track-record" className="py-32 px-6 z-10">
      <div className="max-w-7xl mx-auto">
        <motion.div
          className={`mb-16 flex flex-col md:flex-row md:items-end justify-between gap-6 ${lang === 'ar' ? 'md:flex-row-reverse text-right' : ''}`}
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7 }}
          viewport={{ once: true }}
        >
          <div>
            ...
            <h2 className="font-heading text-4xl md:text-6xl font-extrabold tracking-tight mt-2">{t.track.title}</h2>
          </div>
    ...
    <section id="team" className="py-32 px-6 bg-secondary/20 z-10">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <h2 className="font-heading text-4xl md:text-6xl font-extrabold tracking-tight mt-4">{t.team.title}</h2>
        </motion.div>
          ...
          {leaders.map((p, i) => (
            ...
            >
              <div className="aspect-[4/5] rounded-[2rem] bg-gradient-to-b from-blue-600/10 to-blue-900/20 border border-border/50 overflow-hidden mb-6 group-hover:border-blue-500/50 transition-all duration-500">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-8 text-center">
              ...
              <div className="text-center group-hover:opacity-0 transition-opacity duration-300">
                ...
                <p className="font-sans text-muted-foreground text-[10px] uppercase tracking-widest">{p.role}</p>
              </div>
            </motion.div>
          ))}
        </div>
    ...
    <section className="py-32 px-6 z-10">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <h2 className="font-heading text-4xl md:text-6xl font-extrabold tracking-tight mt-4">{t.values.title}</h2>
        </motion.div>

        <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 ${lang === 'ar' ? 'text-right' : ''}`}>
          {valueItems.map((v, i) => {
            const Icon = v.icon;
            return (
              <motion.div
    ...
    <section id="contact" className="relative py-40 px-6 z-10 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10">
        ...
        >
          ...
          <h2 className="font-heading text-[clamp(2rem,6vw,5rem)] font-extrabold tracking-tight mt-4 mb-8 leading-tight">
            {t.cta.title.split(' ').map((w: string, i: number) => i > 1 ? <span key={i} className="text-gradient-blue italic">{w} </span> : w + ' ')}
          </h2>
          <p className="font-sans text-muted-foreground text-xl mb-12 max-w-xl mx-auto leading-relaxed">
            {t.cta.desc}
          </p>
          
          <div className={`flex flex-col md:flex-row items-center justify-center gap-3 max-w-md mx-auto mb-10 ${lang === 'ar' ? 'md:flex-row-reverse' : ''}`}>
            <input 
              type="email" 
              placeholder={t.cta.placeholder}
              className={`w-full bg-secondary/50 border border-border/50 rounded-full px-6 py-4 outline-none focus:border-blue-500/50 transition-colors ${lang === 'ar' ? 'text-right' : ''}`}
            />
            <button className="btn-primary whitespace-nowrap px-8 h-full py-4 rounded-full">{t.cta.btn}</button>
          </div>

          <div className={`flex justify-center gap-8 text-sm font-bold text-blue-500 ${lang === 'ar' ? 'flex-row-reverse' : ''}`}>
            <a href="mailto:info@mubashercapital.com" className="hover:opacity-70 transition-opacity">{t.cta.link1}</a>
            <a href="#" className="hover:opacity-70 transition-opacity">{t.cta.link2}</a>
          </div>
        </motion.div>
      </div>
    </section>
...
function Footer({ t, lang }: { t: any, lang: string }) {
  ...
  return (
    <footer className={`relative z-10 bg-background border-t border-border/40 pt-24 pb-12 px-6 ${lang === 'ar' ? 'text-right' : ''}`}>
      <div className="max-w-7xl mx-auto">
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-16 mb-20 ${lang === 'ar' ? 'lg:flex lg:flex-row-reverse lg:justify-between' : ''}`}>
          <div className="lg:col-span-2">
            <a href="/" className={`flex items-center gap-2.5 mb-8 no-underline ${lang === 'ar' ? 'flex-row-reverse' : ''}`}>
          ...
          {footerLinks.map(col => (
            <div key={col.title}>
              <h4 className="font-heading font-bold text-[10px] tracking-widest uppercase mb-8 text-foreground">{col.title}</h4>
        ...
        <div className={`pt-8 border-t border-border/30 flex flex-col md:flex-row justify-between items-center gap-6 text-[10px] tracking-[0.2em] font-bold text-muted-foreground/60 uppercase ${lang === 'ar' ? 'md:flex-row-reverse' : ''}`}>
          <span>&copy; {new Date().getFullYear()} {t.footer.rights}</span>
          <div className={`flex gap-10 ${lang === 'ar' ? 'flex-row-reverse' : ''}`}>
            <span>{lang === 'en' ? 'Cairo' : 'القاهرة'}</span>
            <span>{lang === 'en' ? 'Dubai' : 'دبي'}</span>
            <span>{lang === 'en' ? 'Riyadh' : 'الرياض'}</span>
          </div>
        </div>
      </div>
    </footer>
...
export default function Home() {
  ...
  return (
    <div className={`min-h-screen bg-background text-foreground selection:bg-blue-500/30 ${lang === 'ar' ? 'font-arabic' : ''}`}>
      <Cursor />
